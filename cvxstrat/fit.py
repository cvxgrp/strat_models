...
def fit_stratified_model(L, shape, l_prox, r_prox, data=dict(), abs_tol=1e-3, rel_tol=1e-3,
    ...
    Args:
        ...
        - data (optional): dictionary of warm starting values. (default=dict())
    ...
    if 'theta_init' in data:
        theta = data['theta_init'].copy()
    else:
        theta = np.zeros((K,) + shape)
    if 'theta_tilde' in data:
        theta_tilde = data['theta_tilde'].copy()
    else:
        theta_tilde = theta.copy()
    if 'theta_hat' in data:
        theta_hat = data['theta_hat'].copy()
    else:
