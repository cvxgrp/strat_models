...
def mean_cov_prox_torch(Y, eta, theta, t):
	Y = Y[0]
	n, N = Y.shape
	ybar = np.mean(Y,1)
	Y = Y @ Y.T / N
	...
	ybar_tch = torch.from_numpy(ybar)
	Y_tch = torch.from_numpy(Y)

	S0_pd = np.random.randn(n,n)
	S0_pd = np.cov(S0_pd)

	S = torch.tensor(S0_pd, requires_grad=True)
	nu = torch.tensor(np.random.randn(n), requires_grad=True)

	optim = torch.optim.LBFGS([S, nu], lr=.1, max_iter=1000, tolerance_grad=1e-2,
	                         history_size=100)
	...
	#     w,v = torch.symeig(S, eigenvectors=True)
	#     w = torch.tensor((np.maximum(w.data.numpy(), 1e-12)))
	#     SS = v @ torch.diag(w) @ v.t()    

	    main_part = 0
	    main_part += -N*torch.logdet(S)
	    main_part += N*torch.trace(S@Y_tch)
	    main_part += -2*N*ybar_tch @ nu
	    
	    # Sinv, _ = torch.solve(0+torch.eye(n, dtype=torch.float64), SS)
	    Sinv = S.inverse()
	    
	    # print( (nu.reshape((n,1)).t()@Sinv@nu)[0][0] )

	    main_part += N*(nu.reshape((n,1)).t()@Sinv@nu)[0]
	    
	    prox_part = (1./(2*t)) * torch.norm(S - T_tch)**2 + (1./(2*t)) * torch.sum((nu - t_tch)**2)

	    loss = main_part + prox_part
	...
	return np.hstack((S.detach().numpy(), nu.detach().numpy().reshape(-1,1)))
...
def mean_cov_prox_lbfgs(Y, eta, theta, t):
	...
	def loss_fcn(Snu):
		...
		if not np.all(np.linalg.eigvals(Snu[:,:-1]) >= 0):
			return float(np.inf)
		...
		prox_part = eps*np.linalg.norm(S-T, "fro")**2 + eps*np.linalg.norm(nu-tt,2)**2
	...
	Snu_0 = np.hstack((Yemp, ybar)).reshape(-1)
...
def mean_cov_prox_proxgrad(Y, eta, theta, t):
	if Y is None:
		return eta
	Y = Y[0]
	N = Y.shape[1]
	...
	def f(Snu, inv_S):
		S = Snu[:,:-1]
		nu = Snu[:,-1].reshape(-1,1)    
		main_part = -N*np.linalg.slogdet(S)[1] + N*np.trace(S@Yemp) - N*2*ybar.T @ nu
		main_part += N*nu.T @ inv_S @ nu
		
		prox_part = eps*np.linalg.norm(S-T, 'fro')**2 
		prox_part += eps*np.linalg.norm(nu - tt,2)**2
		return float(main_part+prox_part)

	def grad_f(Snu, inv_S):
		S = Snu[:,:-1]
		nu = Snu[:,-1].reshape(-1,1)

		grad_S = -N*inv_S + N*Yemp - N*(inv_S @ nu @ nu.T @ inv_S).T + 2*eps*(S-T)
		grad_nu = -2*N*ybar + 2*N*inv_S@nu + 2*eps*(nu-tt)
		
		return np.hstack((grad_S, grad_nu))

	def project_onto_PSD(Snu):
		S = Snu[:,:-1]
		w, v = np.linalg.eigh(S)
		Snu[:,:-1] = v @ np.diag(np.maximum(w,0)) @ v.T
		return Snu

	def f_approx(Z, Snu, inv_Snu, t):
		main_part = f(Snu, inv_Snu) + np.trace(grad_f(Snu, inv_Snu).T @ (Z - Snu))
		return main_part + (1./(2*t))*np.linalg.norm(Z-Snu,"fro")**2

	Snu = np.hstack((Yemp, ybar))
	Snu_prev = np.hstack((Yemp, ybar))
	# Snu_prev = np.copy(Snu)
	num_iter = 1000
	...
	# # 	Snu_y = Snu + omega_k * (Snu - Snu_prev)
	# # # 	Snu_y = Snu
		
	# # 	Snu_prev = np.copy(Snu)
	# # 	Snu_y_inv = np.linalg.inv(Snu_y[:,:-1])

	# # 	Snu = project_onto_PSD(Snu_y - lambd*grad_f(Snu_y, Snu_y_inv))

	# # 	###Line search
	# # 	while True:
	# # 		z = project_onto_PSD(Snu_y - lambd*grad_f(Snu_y, Snu_y_inv))
	# # 		z_inv = np.linalg.inv(z[:,:-1])

	# # 		if f(z, z_inv) <= f_approx(z, Snu_y, Snu_y_inv, t=lambd):
	# # 			break
	# # 		lambd *= 0.65
	# # 		print(lambd)
	...
	for k in range(1, num_iter+1):
		...
		deltaSnu = -grad_f(Snu, np.linalg.inv(Snu[:,:-1]))

		S_inv = np.linalg.inv(Snu[:,:-1])
		...
		# while True:
		# 	count += 1
		# 	S_plus_delta_inv = np.linalg.inv(Snu[:,:-1] + stepsize*deltaSnu[:,:-1])
		# 	LHS = f(Snu+stepsize*deltaSnu, S_plus_delta_inv)
		# 	RHS = f(Snu, S_inv) - alpha*stepsize*np.linalg.norm(deltaSnu, "fro")**2
		# 	if count >= 100 or LHS < RHS:
		# 		break
		# 	stepsize = beta*stepsize


		Snu = Snu + stepsize*deltaSnu


	# # 	#if k%100 == 0: print(k, np.linalg.norm(Snu-Snu_prev, "fro"))
	# 	if np.linalg.norm(Snu-Snu_prev, "fro") <= 1e-4:
	# 		# print("Terminated in {} iterations".format(k))
	# 		break
		 
	return Snu

	# for k in range(1, num_iter+1):
	# 	f_prev = f(Snu, np.linalg.inv(Snu[:,:-1]))
	# 	Snu_next = project_onto_PSD(Snu - (lambd)*grad_f(Snu, np.linalg.inv(Snu[:,:-1])))

	# 	f_next = f(Snu_next, np.linalg.inv(Snu_next[:,:-1])) 
	...
	# 	# if np.linalg.norm(Snu-Snu_prev, "fro") <= 1e-3:
	# 	# 	# print("Terminated in {} iterations".format(k))
	# 	# 	break
